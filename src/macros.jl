...
# ------------------------------------------------------------------
function _install_overlay(fncall::Expr)
    @assert fncall.head === :call
    fn   = fncall.args[1]
    args = fncall.args[2:end]
...
# ------------------------------------------------------------------
#  @pre  — identical public syntax, now installs overlay automatically
# ------------------------------------------------------------------


"""
From a function definition extract the function call
...
macro pre(precond, msg = "")
    ...
    ov   = Spec._install_overlay(fncall)
...
# ------------------------------------------------------------------
# @invariant stays untouched (uses original Cassette‑less machinery)
# ------------------------------------------------------------------
